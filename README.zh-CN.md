...
Eval Floor 把这个偏差算出来，也就是你的搜索白拿的那条地板线，
...
要传全部变体，不是只传赢的那个。候选数量是决定地板高低的一半因素。
...
大多数工具在这里会打印"没有提升"。那是错的。不是这个改动失败了，是你现在还判不出来。5 个不一致对的精确符号检验下限就是 0.0625，
无论多一边倒都到不了 0.05。
...
所以 5 个点的涨幅在搜索白拿的范围之内。

**「这和留出集有什么区别？」**
它不是替代品，是留出集之前的那一步。地板只用你手上已有的数据，
就能告诉你这次搜索什么也没证明；留出集才能告诉你它证明了什么，那是 `confirm()` 做的事。

**「这不就是过拟合评测集吗？」**
同一类，但机制不同。过拟合通常指模型记住了样本；
这里是选择偏差，你什么都没拟合，只是在几个带噪测量里挑了最大的那个。

**「我的指标不是准确率。」**
`selection_floor` 假设指标是二项型。指标无界或重尾时真实地板更高，
所以过不了检验的提升仍然过不了。

**「我的循环是分级的，便宜档筛完再上贵档。」**
用 `staged_floor()`。地板通常是便宜那一级的，不是你花钱买的贵档。见上文。

## 说明

这里的统计不新：胜者诅咒、选择性推断、best-of-k 的期望值，都是老东西。
新的只是你能用一行拿到你自己那次运行的数。
